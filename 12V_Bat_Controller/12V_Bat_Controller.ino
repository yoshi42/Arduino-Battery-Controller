...
void check_voltage() {
  ...
  if (input_voltage_bat < 13.0 && input_voltage_bat > 11.5 && button_flag == true)  //якщо акум норм - зарядка викл (навантаження вкл)
  {
    status = true;
    digitalWrite(relay, status);
  }

  if (button_flag == true && input_voltage_bat < 13.0 && input_voltage_bat > 11.5 && input_voltage_charge > 5)  // якщо батарея норм, є живлення ЗУ, але захотілося підзарядити - натисни кнопку
  {
    button_flag = false;
    status = true;
    digitalWrite(relay, status);
  ...
  Serial.print(";  v_chrg = ");
  Serial.print(input_voltage_charge);
